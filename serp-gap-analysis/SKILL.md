...
2. **Content gap analysis** (subagent) — Ref: `references/content-gap-process.md`. Call `analyze_serp(kw, keyword_library, mode: "content_gap_analysis")`, poll `get_research_status(task_id)` until done, present result. Output template: `templates/phase-2-content-gap.md`. Returns: must-have topics, target length/format, weakness analysis.
